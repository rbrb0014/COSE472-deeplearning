...
\begin{algorithm}[tb]
   ...
   \label{alg:gravity}
\begin{algorithmic}
   \STATE {\bfseries Input:} parameter matrix array $params$, gradient $grad$, learning rate $lr$
   \STATE S=0
   \REPEAT
...
\begin{algorithm}[tb]
   ...
   \label{alg:cosine}
\begin{algorithmic}
   \STATE {\bfseries Input:} parameter matrix array $params$, gradient $grad$, saver $m$, cosine value $cosine$, learning rate $lr$, cosine constant $C$
   \REPEAT
   \STATE for param in params
   \FOR {$param$ {\bfseries in} $params$}
   \FOR {$m, g, cos$ {\bfseries in} $param.m, param.gradient, param.cosine$}
